...
public class CutString {
    /**
     * 动态规划版本 f(n)定义为将长度为n的绳子分成若干段后的各段长度的最大乘积（最优解），在剪第一刀时有n-1种剪法，可选择在0 < i <
     * n处下刀 在i处下刀，分成长度为i的左半绳子和长度为n-i的右半绳子，对于这两根绳子，定义最优解为f(i)和f(n-i)，于是f(n) =
     * max(f(i) * f(n-i))，即求出各种相乘可能中的最大值就是f(n)的最优解
     * 就这样从上到下的分下去，但是问题的解决从下到上。即先求出f(2)、f(3)的最优解，然后根据这两个值求出f(4)、f(5)...直到f(n)
     * f(2) = 1，因为只能分成两半 f(3) = 2，因为分成两段2*1 大于分成三段的1*1*1 ...
     */
    ...
    /**
     * 贪婪法，不断分出长度为3的绳子，如果最后只剩下长度为1的绳子，退一步，将得到长度为4的绳子，然后将这长度为4的绳子分成2*2(这样分是因为2*2
     * 大于原来的3*1) 因此n = 4时不能分出长度为3的绳子,而n = 2，n = 3可直接返回 当n >=5时候，满足n
     * >=5这个不等式的有2*(n-2) > n以及3*(n-3) > n 注意到2+n-2 = 3+n-3 =
     * n,也就是说分出的两个相乘的数要满足和为n，且同样的n，3*(n-3)的值更大，这就是为什么要不断分出长度为3的绳子的原因
     */

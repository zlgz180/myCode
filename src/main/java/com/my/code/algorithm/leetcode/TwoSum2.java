...
/**
 * 有一个无序int数组，没有重复元素，求出把数组分成三份的两个数最小的和。举例：
 * 1、2、3、4、5、6，最小和是2 + 4 = 6，时间复杂度O(n)。
 */
...
//        System.out.println(Arrays.toString(twoSum(nums)));
    ...
    private static long getMinSum(int arr[]) {
        if (arr == null || arr.length < 5) return -1;   // or throw npe

        int leftIndex = 1, rightIndex = arr.length - 2;
        long leftMin = arr[leftIndex], rightMin = arr[rightIndex];

        while (leftIndex + 2 < rightIndex) {
            if (leftMin < rightMin) rightMin = Math.min(rightMin, arr[--rightIndex]);
            else leftMin = Math.min(leftMin, arr[++leftIndex]);
        }
